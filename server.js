...
MongoClient.connect(URL, function(err, db) {
  ...
  app.get('/new/https?://*.*', function(req, res) {
    ...
    collection.find({original_url: req.originalUrl.slice(5)}, {_id: 0, original_url: 1, short_url: 1}).toArray(function(err, docs) {
      if (err) throw err;
      if (docs.length < 1) {
        ...
        collection.insert({original_url: req.originalUrl.slice(5), short_url: "https://" + req.headers.host + "/" + randNum}, function(err) {
          if (err) return console.log(err);
          collection.find({original_url: req.originalUrl.slice(5)}, {_id: 0, original_url: 1, short_url: 1}).toArray(function(err, docs) {
            if (err) throw err;
  ...
  app.get('/new/:something', function(req, res) {
    //If the url format is wrong, display a json error
    console.log("Invalid url inserted");
    res.send({error: "Wrong url format, make sure you have a valid protocol and real site."});
  });
